...
int main() {
	...
	//ititally assigns the size of the arrays
	...
	//loop to process every image
	for (int i = 0; i < MAX_IMAGES; ++i) {
		char input_filename[100];
		char output_filename_blurred[100];
		char output_filename_edge[100];

		// Create file names dynamically
		sprintf_s(input_filename, IN, i);
		sprintf_s(output_filename_blurred, OUT, i);
		sprintf_s(output_filename_edge, OUT2, i);
		...
		write_image2(output_filename_blurred, filt);
		write_image2(output_filename_edge, gradient);
	}
